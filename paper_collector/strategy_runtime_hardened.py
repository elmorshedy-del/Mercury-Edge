...
Everything else can still be simulated aggressively in the independent shadow
lab, preserving rapid learning without contaminating benchmark P&L.
"""
...
def _expand_hsr_routes(bundle: se.Bundle, strategy_cfg: dict[str, Any]) -> None:
    if bundle.strategy_code != "HSR":
        return
    config = strategy_cfg.get("config", {}) if isinstance(strategy_cfg.get("config"), dict) else {}
    dead = [str(x) for x in bundle.evidence.get("dead_markets", []) if x]
    if not dead:
        return
    max_price = se.d(config.get("max_entry_price", 0.97))
...
def execute_extra_strategies(
    ...
    if same_day_high is None:
        return 0

    guarded_weather = dict(weather)
    guarded_weather["max_temperature_f"] = same_day_high.value_f
    ...
    for event in events:
        ...
        if configs.get("WTY", {}).get("enabled"):
            item = se.construct_wty(conn, session_id, guarded_weather, meta, event, configs["WTY"]["config"])
            if item:
                bundles.append(item)
        ...
        if configs.get("RMO", {}).get("enabled"):
            ...
            rmo = _validate_rmo_prior(
                conn,
                se.construct_rmo(conn, session_id, guarded_weather, meta, event, rmo_cfg),
                guarded_weather,
                timezone_name,
                rmo_cfg,
            )
        ...
        prvs = (
            se.construct_prv(guarded_weather, meta, event, configs.get("PRV", {}).get("config", {}))
            if configs.get("PRV", {}).get("enabled") else []
        )
        bundles.extend(prvs)

        # Sequential strategies are still generated prospectively, but their
        # paper benchmark eligibility is decided only after signal creation.
        if rmo and configs.get("LVP", {}).get("enabled"):
            leg = rmo.legs[0]
            bundles.append(se._bundle(
                "LVP", "sequential_hypothesis", guarded_weather, meta, event,
        ...
        if rmo and configs.get("HMF", {}).get("enabled"):
            leg = rmo.legs[0]
            bundles.append(se._bundle(
                "HMF", "sequential_hypothesis", guarded_weather, meta, event,
        ...
        if proven and configs.get("HMF", {}).get("enabled"):
            ...
            if prior:
                ...
                if dead:
                    ...
                    if ticker:
                        bundles.append(se._bundle(
                            "HMF", "hard_state_sequence", guarded_weather, meta, event,
                            [se.Leg(
